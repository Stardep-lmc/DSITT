...
\title{DSITT: Query-Level Cross-Modal Fusion via\\Modality-Temporal Unified Queries for RGBT Tiny Object Tracking}
...
We propose \textbf{DSITT}, built on \textit{Modality-Temporal Unified Queries} (MTUQ)---a structured representation where each query maintains four views: $\mathbf{q}_\text{rgb}$, $\mathbf{q}_\text{ir}$, $\mathbf{q}_\text{mot}$, $\mathbf{q}_\text{fused}$.
...
Experiments on RGBT-Tiny demonstrate that DSITT achieves state-of-the-art performance.
\end{abstract}
...
To the best of our knowledge, \textbf{DSITT is the first end-to-end MOT framework that jointly addresses} (1) multi-modal RGBT fusion, (2) tiny target optimization, and (3) temporal tracking in a unified query-centric design.
...
DSITT extends this paradigm with structured multi-view queries for RGBT fusion.
...
Table~\ref{tab:position} summarizes DSITT's unique positioning.

\begin{table}[t]
\centering
\caption{Positioning of DSITT vs.\ related methods.}
\label{tab:position}
...
\midrule
\textbf{DSITT (Ours)}      & \textbf{MOT} & \textbf{RGBT} & \textbf{Query} & \cmark & \cmark \\
\bottomrule
\end{tabular}}
\end{table}
...
\FIGNEEDED{%
\textbf{Fig.~2.} Overall DSITT architecture. Dual-stream ResNet-50 backbones $\to$ independent deformable encoders $\to$ MTUQ query construction $\to$ MAD decoder (6 layers) $\to$ prediction from $\mathbf{q}_\text{fused}$. Show CMC auxiliary heads on $\mathbf{q}_\text{rgb}$/$\mathbf{q}_\text{ir}$, motion memory bank feeding $\mathbf{q}_\text{mot}$, and all four query views labeled.
}
%% [需要手绘] 完整架构图

\subsection{Architecture Overview}

Given RGBT frame pairs $(\mathbf{I}_\text{rgb}^t, \mathbf{I}_\text{ir}^t)$ for $t=1,\ldots,T$, DSITT proceeds in five stages:
...
\midrule
\textbf{DSITT (Ours)} & RGBT & \textbf{\TODO{}} & \textbf{\TODO{}} & \textbf{\TODO{}} & \textbf{\TODO{}} & \textbf{\TODO{}} \\
\bottomrule
\end{tabular}}
\end{table}
...
DSITT w/o CMC         & \TODO{} & \TODO{} & \TODO{} & \TODO{} & \TODO{} \\
\textbf{DSITT (full)} & \textbf{\TODO{}} & \textbf{\TODO{}} & \textbf{\TODO{}} & \textbf{\TODO{}} & \textbf{\TODO{}} \\
\bottomrule
\end{tabular}}
\end{table}
...
\FIGNEEDED{%
\textbf{Fig.~5.} Qualitative tracking comparison. Top: MOTR baseline (single-modality) with ID switches. Bottom: DSITT with stable tracking on challenging sequences.
}
%% [需要手绘] 跟踪轨迹对比可视化
...
\textbf{DSITT} & 81.8M & \TODO{} & \TODO{} & \TODO{} \\
\bottomrule
\end{tabular}
\end{table}
...
We presented DSITT, the first end-to-end RGBT multi-object tracker for tiny targets. Our core contribution is \textit{query-level cross-modal fusion} via Modality-Temporal Unified Queries (MTUQ), which defers fusion from the feature level to inside the decoder. The Modality-Aware Decoder (MAD) enables iterative cross-modal reasoning per query, while the Cross-Modal Consistency (CMC) loss provides a novel self-supervision signal improving robustness under modality degradation. NWD-based optimization and Scale-Adaptive Sampling further address tiny-target challenges.
...
Extending DSITT to other multi-modal settings (RGB-LiDAR, RGB-event) and larger-scale MOT benchmarks is a natural next step.
