...
\end{equation}
with $r_\text{max}=0.5$.
A diversity loss $\mathcal{L}_\text{div}=\max(0,\;\sigma_\text{target} - \text{std}(\{s_i\}_{i=1}^N))$ prevents scale collapse, where $\sigma_\text{target}=0.15$ is a target standard deviation threshold.
